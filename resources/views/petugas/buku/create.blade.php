...
@section('title')
    Tambah Buku - Readbook    
@endsection

@section('content')
<div class="my-4 card w-50 mx-auto border-0 shadow-sm">
    <div class="card-body">
        <div class="ps-3 fw-bold text-primary fs-5">Tambah Buku</div>
        <form action="/petugas/buku/tambah" method="POST" class="p-3" enctype="multipart/form-data">
            @csrf
            <div class="pb-3 position-relative">
                <label for="judul" class="form-label">Judul</label>
                <input type="text" name="judul" id="judul" class="form-control mb-1 {{ $errors->has('judul') ? 'is-invalid' : ''}}" placeholder="Enter judul here..." aria-label="judul" autocomplete="false" value="{{old('judul')}}">
                @error('judul')
            ...
            <div class="pb-3 position-relative">
                ...
                <input type="date" name="tahun_terbit" id="tahun_terbit" class="form-control mb-1 {{ $errors->has('tahun_terbit') ? 'is-invalid' : ''}}" placeholder="Enter tahun_terbit here..." aria-label="tahun_terbit" value="{{old('tahun_terbit')}}">
                @error('tahun_terbit')
                    <div class="invalid-feedback position-absolute bottom-0 ps-2">{{$message}}</div>
                @enderror
            </div>
            <div class="pb-3 position-relative">
                ...
                <input type="number" name="jumlah" id="jumlah" class="form-control mb-1 {{ $errors->has('jumlah') ? 'is-invalid' : ''}}" placeholder="Enter jumlah here..." aria-label="jumlah" value="{{old('jumlah')}}">
                @error('jumlah')
                    <div class="invalid-feedback position-absolute bottom-0 ps-2">{{$message}}</div>
                @enderror
            </div>
            <div class="pb-3 position-relative">
                ...
                <input type="text" name="pengarang" id="pengarang" class="form-control mb-1 {{ $errors->has('pengarang') ? 'is-invalid' : ''}}" placeholder="Enter pengarang here..." aria-label="pengarang" value="{{old('pengarang')}}">
                @error('pengarang')
                    <div class="invalid-feedback position-absolute bottom-0 ps-2">{{$message}}</div>
                @enderror
            </div>
            ...
            <div id="tags" class="d-flex flex-wrap align-items-start border-bottom py-4"></div>
